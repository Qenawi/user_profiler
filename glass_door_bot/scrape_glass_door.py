...
def get_jobs(keyword, location, num_jobs, verbose, slp_time):
    ...
    options = Options()
    path = "/home/ahmedqenawi/linked_in_profiler/geckodriver"
    ...
    search = driver.find_element_by_id("sc.location").clear()
    search = driver.find_element_by_id("sc.location")
    ...
    ## paybass sign up 
    try:
                # list all jobs associted with that job title 
                driver.find_element_by_xpath("/html/body/div[2]/div/div/div[1]/div[1]/div[3]/a").click()
    ...
    #time.sleep(5)
    #driver.find_element_by_xpath('/html/body/div[2]/div/div/div/div/div[1]/header/div[2]/ul/li[4]').click()
    ...
    while len(jobs) < num_jobs:  #If true, should be still looking for new jobs.
        ...
        job_buttons = driver.find_elements_by_class_name("react-job-listing")  #for Job Listing. These are the buttons we're going to click.
        for job_button in job_buttons:  
            ...
            print("Qenawi -->> \n" )
            i = job_button.get_attribute("location")
            print(i)
            print("Qenawi <-- \n" )
  
            while not collected_successfully:
                try:
                    company_name = "company name" #job_button.text
                    location = "location" #driver.find_element_by_xpath('.//div[@class="location"]').text
                    job_title = "job_title" #driver.find_element_by_xpath('.//div[contains(@class, "title")]').text
...
## main
# 
a = get_jobs('android developer', 'Cairo, Egypt', 120, True, 2)
a.to_csv()